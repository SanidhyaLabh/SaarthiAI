# 👁️ SaarthiAI (सारथी AI) - Mobile Spatial Vision Assistant
...
### 1. 🎙️ Zero-Touch Voice Assistant (Instant Startup)
...
- 👁️ **Scene Description**: Rich panoramic spatial description detailing layout, obstacles, lighting, and clear walking paths.
...
### 4. 🎛️ Universal Tactile & Hardware Accessibility (WCAG AAA)
...
```bash
git clone https://github.com/SanidhyaLabh/SaarthiAI.git
cd SaarthiAI
```
...
npm run dev
```
Open `http://localhost:3000` (Switch to Mobile Viewport in browser DevTools: 390x844).
...
### Deploy on Render (Recommended for Full-Stack Node + Express)
1. Push your repository to GitHub.
2. Log in to [Render Dashboard](https://dashboard.render.com/) and create a **New Web Service**.
3. Connect your `SaarthiAI` repository.
...
### Deploy with Docker
```bash
docker build -t saarthi-ai .
docker run -p 5000:5000 -e GEMINI_API_KEY="your_api_key_here" saarthi-ai
```
